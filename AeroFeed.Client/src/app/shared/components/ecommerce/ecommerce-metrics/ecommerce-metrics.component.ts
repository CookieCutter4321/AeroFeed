...
export class EcommerceMetricsComponent implements AfterViewInit {
  public chartOptions: ChartOptions;
  public isHovering: boolean = false;
  @ViewChild('pie_chart') chart!: ChartComponent;
  ...
  constructor(public kafkaService: KafkaDataService) {
    this.chartOptions = {
      ...
      chart: {
        ...
        events: {
          dataPointMouseEnter: () => this.isHovering = true,
          dataPointMouseLeave: () => this.isHovering = false,
        }
      },
      labels: ["categorize", "edit", "external", "log", "new"],
      responsive: [
        {
          breakpoint: 480,
          options: {
  ...
  ngAfterViewInit(): void {
    this.kafkaService.currentData$.subscribe((data) => {
      if (data == null || data.typeCounts == null || this.isHovering) return;
      let x: number[] = Object.values(data.typeCounts);
